...
class DocumentTokenAnalyzer:
    ...
    def count_page_tokens(self, page_data: Dict[str, Any]) -> Dict[str, int]:
        ...
        return {
            ...
            'block_count': len(page_data.get('lines', [])),  # 'lines' for lines file, 'blocks' for blocks file
    ...
    def analyze_document(
        ...
        remaining_pages = all_pages[random_start_page-1:] if total_pages > random_start_page-1 else []
        random_pages = random.sample(
            remaining_pages, 
            min(random_sample_size, len(remaining_pages))
    ...
    def recommend_batch_sizes(self, stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        conservative_pages = int(available_tokens // max_tokens) if max_tokens > 0 else 0
        optimistic_pages = int(available_tokens // avg_tokens) if avg_tokens > 0 else 0
        recommended_pages = int(available_tokens // conservative_estimate) if conservative_estimate > 0 else 0
        
        # Practical limits and suggestions
        recommended_initial = min(20, recommended_pages)  # Higher limit for GPT-4.1
        recommended_incremental = min(8, recommended_pages // 3)  # Higher incremental for GPT-4.1
