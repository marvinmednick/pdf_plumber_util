...
class TestCLITOCAnalysis:
    ...
    def test_llm_analyze_cost_estimation_with_toc_enhancement(self, mock_analyzer, mock_llm_analyzer_class):
        ...
        with self.runner.isolated_filesystem():
            ...
            assert "2450" in output or "2,450" in output  # Enhanced token count
            assert "0.024" in output or "$0.02" in output  # Cost estimate
            
            # Verify cost estimation was called correctly
            mock_llm_analyzer.estimate_analysis_cost.assert_called_once()

    @patch('src.pdf_plumb.workflow.orchestrator.AnalysisOrchestrator')
    @patch('src.pdf_plumb.core.analyzer.DocumentAnalyzer')
    def test_cli_workflow_integration_with_toc_states(self, mock_analyzer, mock_orchestrator_class):
        """Test CLI integration with complete state machine workflow including TOC analysis.
        
        Test setup:
        - Mock AnalysisOrchestrator with multi-state workflow execution
        - Include HeaderFooterAnalysisState and AdditionalSectionHeadingState
        - Configure realistic workflow results with TOC knowledge accumulation
        
        What it verifies:
        - Complete workflow executes through state machine with TOC enhancement
        - Context accumulation includes TOC patterns from HeaderFooter state
        - Both states contribute TOC-related knowledge to final results
        - Workflow metadata tracks enhanced analysis across all states
        
        Key insight: Validates TOC enhancement integrates properly with complete analysis workflow.
        """
        with self.runner.isolated_filesystem():
            # Create test input
            test_input = Path('test_doc_blocks.json')
            ...
            # Configure workflow results with TOC knowledge
            mock_orchestrator.run_workflow.return_value = {
                'workflow_results': {
                    'header_footer_analysis': {
                        'analysis_type': 'header_footer_analysis',
                        'results': {
                            'header_footer_patterns': {'detected': True},
                            'document_elements': {
                                'section_headings': [{'text': 'Table of Contents'}],
                                'toc_entries': [
                                    {'text': '1. Introduction', 'page_number': '5'},
                                    {'text': '2. Methods', 'page_number': '12'}
                                ]
                            }
                        },
                        'metadata': {
                            'provider_configured': True,
                            'token_usage': {'total_tokens': 2450},
                            'analysis_objectives_completed': 6  # Including TOC
                        }
                    },
                    'additional_section_headings': {
                        'analysis_type': 'additional_section_heading_analysis',
                        'results': {'additional_patterns_found': False}
                    }
                },
                'accumulated_knowledge': {
                    'header_pattern': 'consistent_headers',
                    'footer_pattern': 'page_numbers_center',
                    'toc_structure': 'hierarchical_numbered',
                    'toc_pages_identified': [0],
                    'total_objectives_completed': 6
                },
                'workflow_metadata': {
                    'total_states_executed': 2,
            ...
            assert result.exit_code == 0, f"Workflow analysis failed: {result.output}"
            
            # Validate workflow orchestration was called
    ...
    @patch('src.pdf_plumb.core.llm_analyzer.LLMDocumentAnalyzer')
    @patch('src.pdf_plumb.core.analyzer.DocumentAnalyzer')
    def test_cli_output_file_generation_with_toc_data(self, mock_analyzer, mock_llm_analyzer_class):
        """Test CLI output file generation includes comprehensive TOC data.
        
        Test setup:
        - Configure output directory for result file generation
        - Mock analysis with comprehensive TOC detection results
        - Verify structured output files contain all TOC information
        
        What it verifies:
        - Output JSON files include complete TOC analysis data
        - File structure preserves all 6-objective analysis results
        - TOC-specific fields are properly serialized and accessible
        - Analysis metadata includes enhanced token usage information
        
        Key insight: Ensures TOC enhancement data is properly persisted in output files.
        """
        with self.runner.isolated_filesystem():
            # Create test input
            test_input = Path('test_doc_blocks.json')
            ...
            # Mock LLMDocumentAnalyzer with file saving
            mock_llm_analyzer = Mock()
            mock_llm_analyzer_class.return_value = mock_llm_analyzer
            ...
            def mock_save_results(*args, **kwargs):
                # Simulate saving structured results to output directory
                result_file = output_dir / 'toc_analysis_results.json'
            ...
            mock_llm_analyzer.analyze_headers_footers.side_effect = lambda *args, **kwargs: (
                mock_save_results(),
                toc_result
            )[1]
            
            # Execute analysis with output
            result = self.runner.invoke(cli, [
                'llm-analyze',
                str(test_input),
            ...
            # Validate command success
            assert result.exit_code == 0, f"Analysis with output failed: {result.output}"
            
            # Validate output file was created
            result_files = list(output_dir.glob('*.json'))
            assert len(result_files) > 0, "No output files were created"
            
            # Validate output file contains TOC data
            if result_files:
                with open(result_files[0], 'r') as f:
                    output_data = json.load(f)
                
                # Check for TOC-specific content
                assert 'toc_detection' in str(output_data) or 'table_of_contents' in str(output_data)
